...
QWidget* Homepage::buildLeftNav()
{
    ...
    QPushButton* resultNavBtn = makeNavBtn(ICO_TROPHY, "Game Result", false);
    connect(resultNavBtn, &QPushButton::clicked, this, &Homepage::ViewResultsClicked);
    lay->addWidget(resultNavBtn);
...
QWidget* Homepage::buildMainContent()
{
    ...
    QWidget* exitCardWidget = buildSmallCard(ICO_CLOSE,
                                      "Exit",
                                      "Safely close the application and\nsave your current progress.");
    ClickableCard* exitCard = qobject_cast<ClickableCard*>(exitCardWidget);
    if (exitCard) {
        connect(exitCard, &ClickableCard::clicked, this, &Homepage::ExitClicked);
    }
    cardRow->addWidget(exitCardWidget);
    cardRow->addStretch();
...
QWidget* Homepage::buildStartGameCard()
{
    ...
    connect(play, &QPushButton::clicked, this, &::Homepage::StartGameClicked);
    connect(card, &ClickableCard::clicked, this, &::Homepage::StartGameClicked);
    lay->addWidget(play);
...
QWidget* Homepage::buildGameResultCard()
{
    ...
    btn->setCursor(Qt::PointingHandCursor);
    connect(btn, &QPushButton::clicked, this, &Homepage::ViewResultsClicked);
    connect(card, &ClickableCard::clicked, this, &Homepage::ViewResultsClicked);
    lay->addWidget(btn);
